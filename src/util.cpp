...
const char * const BITCOINBAM_CONF_FILENAME = "bitcoinbam.conf";
const char * const BITCOINBAM_PID_FILENAME = "bitcoinbamd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bitcoinbamd -foo=bar
...
#else
    const char* pszModule = "bitcoinbam";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitcoinBam
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitcoinBam
    // Mac: ~/Library/Application Support/BitcoinBam
    // Unix: ~/.bitcoinbam
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinBam";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitcoinBam";
#else
    // Unix
    return pathRet / ".bitcoinbam";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BITCOINBAM_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", BITCOINBAM_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Bitcoin Core") == std::string::npos) {
        ...
        strYear.replace(strYear.find("2020"), sizeof("2020")-1, "2009-2020");
