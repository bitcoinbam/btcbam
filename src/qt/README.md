This directory contains the BitcoinBamQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the BitcoinBam UI.
...
ClientModel is used by the main application `bitcoinbamgui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/bitcoinbam/bitcoinbam/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a bitcoinbam:... link from a browser).
...
* UI elements like BitcoinBamAmountField, which inherit from QWidget.
* `bitcoinbamstrings.cpp`: automatically generated
* `bitcoinbamunits.(h/cpp)`: BTCBAM / mBTCBAM / etc handling
* `callback.h`
...
* don't change `local/bitcoinbam_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "bitcoinbam-qt" under "Run", which is where you can also add command line arguments)
