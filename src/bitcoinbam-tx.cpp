...
static void SetupBitcoinBamTxArgs()
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " bitcoinbam-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  bitcoinbam-tx [options] <hex-tx> [commands]  Update hex-encoded bitcoinbam transaction\n" +
            "or:     bitcoinbam-tx [options] -create [commands]   Create hex-encoded bitcoinbam transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bitcoinbam transaction
