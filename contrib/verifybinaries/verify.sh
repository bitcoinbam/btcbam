...
###   bitcoinbamcore.org and bitcoinbam.org and compares them.
...
WORKINGDIR="/tmp/bitcoinbam_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoinbamcore.org"
HOST2="https://bitcoinbam.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcoinbam-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoinbam-',
...
if [ $? -ne 0 ]; then
   echo "bitcoinbam.org failed to provide signature file, but bitcoinbamcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoinbam.org and bitcoinbamcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BitcoinBam Core binary release signing key installed?"
   fi
